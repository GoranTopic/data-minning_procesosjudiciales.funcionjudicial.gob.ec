...
let scrap_cedula = async (cedula, axios_instance, logger) => {
    // make an axios instance
    //let axios_instance = make_axios_instance(proxy, userAgent)
    await logger.log('made axios instance')
    ...
    await logger.log(`numero de causas como actor: ${numero_de_causas_como_actor}`)
    ...
    let causas_como_demandado_encontradas
        = await contar_causas(user_search_query_data, axios_instance)
    await logger.log(`numero de causas como demandado: ${causas_como_demandado_encontradas}`)
    ...
    if( causas_actor_scraped && causas_demandado_scraped ){
        await logger.log(`cedula scrapped`)
